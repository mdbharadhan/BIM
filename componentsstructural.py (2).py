...
def render_structural():
    st.markdown("## 🦴 Structural Integrity & Asset Health")[cite: 10]
    st.caption("Sensors and inspection logs monitoring columns, beams, load-bearing walls, and slabs.")[cite: 10]

    conn = get_connection()[cite: 10]
    ...
    if df_struct.empty:
        st.info("No structural elements logged.")[cite: 10]
    else:
        # Display elements
        for _, row in df_struct.iterrows():[cite: 10]
            health = row['health_index'][cite: 10]
            color = "#22C55E" if health >= 90 else "#F59E0B" if health >= 75 else "#EF4444"[cite: 10]
