...
async def test_get_room_not_found_raises_404(db_session: AsyncSession):
    room_service = RoomService(db_session)
    with pytest.raises(HTTPException) as exc_info:
        await room_service.get_room(999)
    assert exc_info.value.status_code == 404


async def test_update_room_not_found_raises_404(db_session: AsyncSession):
    room_service = RoomService(db_session)
    with pytest.raises(HTTPException) as exc_info:
        await room_service.update_room(999, RoomUpdate(occupancy=10))
    assert exc_info.value.status_code == 404


async def test_delete_room_not_found_raises_404(db_session: AsyncSession):
    room_service = RoomService(db_session)
    with pytest.raises(HTTPException) as exc_info:
